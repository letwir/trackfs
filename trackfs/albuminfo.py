...
# and Modified 2024-2025 by letwir
# All rights reserved.
...
class AlbumInfo:
    ...
    def _cue_from_external_file(self) -> Optional[str]:
        ...
        if cue_path is None:
            return None
        log.debug("found accompanying cue sheet")
    ...
    def cue(self) -> Optional[cuesheet.CueSheet]:
        ...
        if len(raw_cue) == 0:
            log.debug("regular flac file without cue sheet")
        ...
        try:
            ...
            log.warning(f'could not parse cue sheet; ignore cue sheet:{raw_cue}')
            return None
    ...
    def _album_tags(self):
        ...
        for (k, v) in (meta.tags if meta.tags else {}):
            ...
            if AlbumInfo.IGNORE_TAGS_REX.match(k):
                continue
            if k not in tags_album: tags_album[k] = []
            tags_album[k].append(v)

        # make sure ALBUMARTIST and ALBUM are set
        # in case ARTIST and TITLE have been used instead
        if 'ALBUMARTIST' not in tags_album and 'ARTIST' in tags_album:
            tags_album['ALBUMARTIST'] = tags_album['ARTIST']
        if 'ALBUM' not in tags_album and 'TITLE' in tags_album:
            tags_album['ALBUM'] = tags_album['TITLE']

        # add missing tags from cue sheet
        for (k, v) in self.cue.tags_album().items():
            if k not in tags_album:
                tags_album[k] = v
    ...
    def track_tags(self, num):
        tags_track = self._album_tags()
        for (k, vs) in self.track(num).tags().items():
            tags_track[k] = vs
        if 'TRACKTOTAL' not in tags_track:
            tags_track['TRACKTOTAL'] = [str(len(self.tracks()))]
        if 'COMPOSER' not in tags_track:
            tags_track['COMPOSER'] = tags_track['ARTIST']
